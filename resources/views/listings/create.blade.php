<x-layout> <x-card class=" p-10 max-w-lg mx-auto mt-24"> <header class="text-center">
  ...
  <form action="/listings" method="POST" enctype='multipart/form-data'> @csrf <div class="mb-6"> <label for="company"
    class="inline-block text-lg mb-2">Company Name</label> <input type="text"
    class="border border-gray-200 rounded p-2 w-full" name="company" value="{{old('company')}}" /> @error('company') <p
    ...
    placeholder="Example: Remote, Boston MA, etc" /> @error('location') <p class="text-red-500 text-xs mt-1">
    {{$message}}
    </p>
    @enderror
    </div>
